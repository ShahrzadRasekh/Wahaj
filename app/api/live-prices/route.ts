...
async function getGoldUsdPerOz() {
  ...
  if (!ok) throw new Error(`Gold API HTTP ${status}: ${text?.slice(0, 120)}`);
...
async function getUsdToAed() {
  ...
  if (!ok) throw new Error(`FX API HTTP ${status}: ${text?.slice(0, 120)}`);
...
function responseOk(goldOz: string, goldG: string, extra?: Record<string, any>) {
...
export async function GET() {
  try {
    ...
    return responseOk(format2(aedPerOz), format2(aedPerG));
  } catch (err: any) {
    // DEV FALLBACK: show mock values so UI works locally (StackBlitz often blocks fetch)
    if (process.env.NODE_ENV !== "production") {
      return responseOk("4,368.09", "517.74", {
        provider: "mock (dev) — external fetch blocked",
        devError: err?.message ?? "fetch failed",
      });
    }

    // PROD: fail gracefully
    return responseOk("--", "--", {
